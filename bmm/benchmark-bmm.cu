...
int main(int argc, char* argv[]) 
{
    ...
    cudaMalloc(&Aconc, M * K);
    cudaMalloc(&Bconc, K * N);
    cudaMemset(fC, 0, M * N * sizeof(float));
    int block = 64, grid = M * K / (block * 32)  + 1;
    int grid1 = K / block;
    dim3 blockDim(16, 16);
    ...
    {
        concatenate_rows_kernel<<<grid, block>>>(fA, Aconc, M * K / 32);
        concatenate_cols_kernel<<<grid1, block>>>(fB, Bconc, K, N);
        xnor_gemm<<<gridDim, blockDim>>>(Aconc, Bconc, fC, M, K / 32, N);
    }
    ...
    cudaFree(Aconc);
    cudaFree(Bconc);
    float* result_xnor = (float*)malloc(M * N * sizeof(float));
    cudaMemcpy(result_xnor, fC, M * N * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for (int i=0; i<TEST_TIMES; i++)
    {
        ...
        BMM32_Arow_Brow<float><<<dim3(M/32,N/32), 32>>>(tA, tB, fC, M, K/32, N);
    }
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    milliseconds = 0;
    ...
    for (int i=0; i<TEST_TIMES; i++)
    {
        ...
        BMM64_Arow_Brow<float><<<dim3(M/64,N/64), 32>>>(llA, llB, fC, M, K/64, N);
    }
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    milliseconds = 0;
    ...
    //============================================= CHECK RESULT
    printf("XNOR success: %d\n", check_result(result_cublas, result_xnor, M, N));
